...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        返回值	                    含义	            用途
        action	                选择的动作  	    执行环境交互
        probs.log_prob(action)	动作的对数概率	     PPO的重要性采样比率计算
        probs.entropy()	        分布的熵值	        鼓励探索，防止策略过早收敛
        self.critic(x)	        状态价值估计	    计算优势函数和价值损失
        """
        logits = self.actor(x)
        probs = Categorical(logits=logits)  # 自动对logits进行softmax处理，得到概率分布
        if action is None:
            action = probs.sample()
